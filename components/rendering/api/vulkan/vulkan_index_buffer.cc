...
VulkanIndexBuffer::VulkanIndexBuffer(VulkanContext& vulkan_context,
                                     VulkanBuffer& vulkan_buffer)
    : vulkan_context_(vulkan_context), vulkan_buffer_(vulkan_buffer) {
...
void VulkanIndexBuffer::Initialize() const { CreateIndexBuffer(); }
...
void VulkanIndexBuffer::CreateIndexBuffer() const {
  ...
  vkMapMemory(vulkan_context_.device, stagingBufferMemory, 0, bufferSize, 0,
              &data);
  memcpy(data, vulkan_context_.indices.data(), static_cast<size_t>(bufferSize));
