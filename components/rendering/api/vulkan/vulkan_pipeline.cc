...
void GraphicsPipeline::Initialize(VulkanContext &vulkan_context) {
...
void GraphicsPipeline::createGraphicsPipeline(VkDevice device) {
  ...
  // Set up the structure that describes the vertex input here...
  ...
  viewport.x = 0.0f;
  viewport.y = 0.0f;
  viewport.width = static_cast<float>(vulkan_context_->swapChainExtent.width);
  viewport.height = static_cast<float>(vulkan_context_->swapChainExtent.height);
  viewport.minDepth = 0.0f;
  viewport.maxDepth = 1.0f;

  VkRect2D scissor{};
  scissor.offset = {0, 0};
  scissor.extent = vulkan_context_->swapChainExtent;
  ...
  if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr,
                             &vulkan_context_->pipelineLayout) != VK_SUCCESS) {
  ...
  pipelineInfo.layout = vulkan_context_->pipelineLayout;
  pipelineInfo.renderPass = vulkan_context_->renderPass;
  pipelineInfo.subpass = 0;
  pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
  pipelineInfo.basePipelineIndex = -1;

  if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                nullptr, &vulkan_context_->graphicsPipeline) !=
      VK_SUCCESS) {
